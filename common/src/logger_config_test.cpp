...
void testConfig() {
    ...
    // Test getInt
    int port = config.getInt("server_port", 8554);
    std::cout << "✓ server_port = " << port << " (expected: 9999)" << std::endl;

    // Test getString
    std::string videoFile = config.getString("video_file", "default.mp4");
    std::cout << "✓ video_file = " << videoFile << " (expected: test_video.mp4)" << std::endl;

    // Test getBool
    bool debug = config.getBool("debug_mode", false);
    std::cout << "✓ debug_mode = " << (debug ? "true" : "false") << " (expected: true)"
              << std::endl;

    // Test getDouble
    double timeout = config.getDouble("timeout_seconds", 5.0);
    std::cout << "✓ timeout_seconds = " << timeout << " (expected: 10.5)" << std::endl;

    // Test hasKey
    bool hasKey = config.hasKey("frame_rate");
    std::cout << "✓ hasKey('frame_rate') = " << (hasKey ? "true" : "false") << " (expected: true)"
              << std::endl;

    // Test default values
    int unknownKey = config.getInt("unknown_key", 123);
    std::cout << "✓ unknown_key (default) = " << unknownKey << " (expected: 123)" << std::endl;
