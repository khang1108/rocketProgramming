...
void RTPPacket::encode() {
    ...
    //  Quy tắc mạng (Big-Endian): Byte lớn (đầu) đi trước, Byte nhỏ (đuôi) đi
    //  sau. Lấy 8 bit đầu (Cao): Dịch phải 8 bước để đẩy 0x12 xuống cuối
...
void RTPPacket::decode(const uint8_t* packet, size_t packetSize) {
    ...
    ssrc_ = (static_cast<uint32_t>(packet[8]) << 24) | (static_cast<uint32_t>(packet[9]) << 16) |
...
void RTPPacket::setVersion(uint8_t v) {
    if (v > 3)
        throw std::invalid_argument("ERROR: Version RTP khong hop le");
    version_ = v;
}

void RTPPacket::setCC(uint8_t cc) {
    if (cc > 15)
        throw std::invalid_argument("ERROR: CC chi duoc tu 0-15");
    cc_ = cc;
}

void RTPPacket::setPayloadType(uint8_t pt) {
    if (pt > 127)
        throw std::invalid_argument("ERROR: Payload Type chi duoc tu 0-127");
    payloadType_ = pt;
}
...
void RTPPacket::printHeader() const {
    ...
    std::cout << "[DEBUG RTP] V=" << (int)version_ << " P=" << (int)padding_
              << " M=" << (int)marker_ << " PT=" << (int)payloadType_ << " Seq=" << sequenceNumber_
              << " TS=" << timestamp_ << " Payload=" << payload_.size() << " bytes" << std::endl;
}
