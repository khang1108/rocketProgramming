...
Frame::Frame(const std::vector<uint8_t> &d, uint16_t seq, uint32_t ts,
            uint32_t src)
=======
#include <algorithm>  // std::min
#include <stdexcept>  // std::runtime_error
...
<<<<<<< HEAD
      hdStrategy_(std::make_unique<HDEncodingStrategy>()), autoDetect_(true) {}

void EncodingContext::setAutoDetect(bool enable) { autoDetect_ = enable; }
...
std::string EncodingContext::getEncodingInfo(const Frame &frame) const {
  ...
  if (frameSize <= RTPPacket::MAX_PAYLOAD_SIZE) {
    return "SD Encoding (single packet): " + std::to_string(frameSize) +
            " bytes";
  }
  else {
    size_t numPackets = (frameSize + RTPPacket::MAX_PAYLOAD_SIZE - 1) /
